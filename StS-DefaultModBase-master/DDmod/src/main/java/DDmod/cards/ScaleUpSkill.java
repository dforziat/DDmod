...
public class ScaleUpSkill extends AbstractDynamicCard {
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        ...
        if(thornsPower == null){
            AbstractDungeon.actionManager.addToBottom(new ApplyPowerAction(p, p, new ThornsPower(p, this.magicNumber), this.magicNumber)); // added one as a default cause feels bad without it
        }else{
            AbstractDungeon.actionManager.addToBottom(new ApplyPowerAction(p, p, new ThornsPower(p, thornsPower.amount * 2), thornsPower.amount));
        }
    }
